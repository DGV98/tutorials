...
///
/// One errdefer after each successful acquisition covers all of it.
fn createPerson(alloc: Allocator, name: []const u8, email: []const u8) !*Person {
    const person = try alloc.create(Person);
    errdefer alloc.destroy(person);

    person.name = try alloc.dupe(u8, name);
    errdefer alloc.free(person.name);

    person.email = try alloc.dupe(u8, email);
    // Nothing can fail after this point, so no errdefer for email is needed.
...
fn destroyPerson(alloc: Allocator, person: *Person) void {
    alloc.free(person.name);
    alloc.free(person.email);
    alloc.destroy(person);
}
...
test "createPerson: no leak no matter which allocation fails" {
    ...
    // counted), then once per allocation forced to fail. Delete ONE errdefer
    // in createPerson and watch this test name the leak.
...
///
///     var filled: usize = 0;
///     errdefer { ...free out[0..filled] and out itself... }
///     for (strings) |s| {
...
fn dupeAll(alloc: Allocator, strings: []const []const u8) ![][]u8 {
    ...
    var filled: usize = 0;
    errdefer {
        for (out[0..filled]) |copy| alloc.free(copy);
        alloc.free(out);
    }
