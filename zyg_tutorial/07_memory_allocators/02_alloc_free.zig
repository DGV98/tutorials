...
fn makeZeroed(alloc: Allocator, n: usize) ![]u8 {
    // TODO: alloc n bytes, @memset them to 0, return the slice.
    // Then delete the discards and the error return below.
    _ = alloc;
    _ = n;
...
/// must fill in every field. Assigning a whole struct literal through the
/// pointer is the cleanest way:
///
///   counter.* = .{ .count = ..., .step = ... };
///
/// The caller destroys it.
fn createCounter(alloc: Allocator, step: u32) !*Counter {
    // TODO: create, initialize, return. Then delete the lines below.
    _ = alloc;
    _ = step;
    return error.NotImplemented;
}
...
/// function. Put `defer alloc.free(scratch)` right after the alloc; it
/// guarantees the scratch is released on every path out of the function.
/// If you forget the defer, the leak report from std.testing.allocator will
/// point at the exact alloc call — try it once on purpose after you're green.
///
/// (Filling: `for (scratch, 0..) |*slot, i|` gives you a write pointer and an
/// index; i is usize, so widen with `@as(u64, i) * @as(u64, i)`.)
fn sumOfSquares(alloc: Allocator, n: usize) !u64 {
    // TODO: alloc scratch, defer free it, fill, sum, return the sum.
    _ = alloc;
    _ = n;
    return error.NotImplemented;
}
