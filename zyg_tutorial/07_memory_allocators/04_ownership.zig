...
fn upperDupe(alloc: Allocator, s: []const u8) ![]u8 {
    // TODO: dupe, upper-case the copy in place, return it.
    _ = alloc;
    _ = s;
...
fn concat(alloc: Allocator, a: []const u8, b: []const u8) ![]u8 {
    // TODO: alloc, two @memcpy calls, return.
    _ = alloc;
    _ = a;
    _ = b;
    return error.NotImplemented;
...
///   return try list.toOwnedSlice(alloc);     // hand the buffer to the caller
///
...
///
/// (Even check: @rem(x, 2) == 0. Plain % rejects signed operands that could
/// be negative — module 01 covered why.)
fn keepEvens(alloc: Allocator, nums: []const i32) ![]i32 {
    // TODO: build up an ArrayList, return via toOwnedSlice.
    _ = alloc;
    _ = nums;
    return error.NotImplemented;
}
