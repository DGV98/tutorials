...
fn numberStrings(alloc: Allocator, n: usize) ![][]u8 {
    // TODO: alloc the outer slice, fill each slot with allocPrint.
    _ = alloc;
    _ = n;
...
/// propagate (a bare `try` does exactly that).
fn greetInto(buf: []u8, name: []const u8) ![]u8 {
    // TODO: FixedBufferAllocator.init(buf), then allocPrint with its
    // .allocator(). Remember: no free needed — the memory is the caller's buf.
    _ = buf;
    _ = name;
    return error.NotImplemented;
}
...
fn splitLines(alloc: Allocator, text: []const u8) ![][]const u8 {
    // TODO: count, alloc, fill from the splitScalar iterator.
    _ = alloc;
    _ = text;
    return error.NotImplemented;
}
