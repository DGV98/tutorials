...
fn minFirst(_: void, a: i64, b: i64) Order {
    // TODO: return the Order that ranks the smaller value first.
    // Hint: std.math.order.
    _ = a; // TODO: remove this discard once you use `a`.
    _ = b; // TODO: remove this discard once you use `b`.
    return .eq;
}
...
fn maxFirst(_: void, a: i64, b: i64) Order {
    // TODO: rank the LARGER value first.
    _ = a; // TODO: remove this discard once you use `a`.
    _ = b; // TODO: remove this discard once you use `b`.
    return .eq;
}
...
pub fn topK(alloc: std.mem.Allocator, xs: []const i64, k: usize) ![]i64 {
    // TODO:
    //   1. var pq: MinQueue = .empty; (+ defer deinit)
    //   2. push each x; if (pq.count() > k) _ = pq.pop();
    //   3. alloc a result slice of pq.count() items and fill it from
    //      the END while popping.
    _ = xs; // TODO: remove this discard once you use `xs`.
    _ = k; // TODO: remove this discard once you use `k`.
