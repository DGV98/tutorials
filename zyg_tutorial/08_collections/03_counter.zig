...
pub fn countWords(alloc: std.mem.Allocator, text: []const u8) !std.StringHashMap(u32) {
    ...
    // TODO: tokenize `text` on " \t\r\n" and apply the getOrPut pattern
    // from the doc comment to each word.
...
pub fn deinitCounter(map: *std.StringHashMap(u32)) void {
    // TODO: before map.deinit(), walk map.keyIterator() and free each
    // key with map.allocator.free(key_ptr.*). Skip this and every duped
    // key leaks — std.testing.allocator will fail the tests for you.
    map.deinit();
}
...
pub fn mostCommon(map: *const std.StringHashMap(u32)) ?WordCount {
    // TODO: walk map.iterator(); entries expose entry.key_ptr.* and
    // entry.value_ptr.*; keep the best one seen.
