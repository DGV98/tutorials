...
pub fn parseNumbers(alloc: std.mem.Allocator, text: []const u8) ![]i64 {
    // TODO:
    //   1. var list: std.ArrayList(i64) = .empty;  (+ defer deinit)
    //   2. Walk std.mem.tokenizeAny(u8, text, ", \n"), parse each token
    //      with std.fmt.parseInt(i64, tok, 10), append it.
    //   3. return try list.toOwnedSlice(alloc);
    _ = text; // TODO: remove this discard once you use `text`.
    return try alloc.alloc(i64, 0);
}
...
pub fn drainStack(list: *std.ArrayList(i64)) i64 {
    // TODO: pop until null, summing as you go.
    _ = list; // TODO: remove this discard once you use `list`.
    return 0;
}
...
pub fn removeAll(list: *std.ArrayList(i64), value: i64, keep_order: bool) usize {
    // TODO: while (i < list.items.len), compare list.items[i] to value,
    // remove with the right method (discard its return: `_ = ...`), and
    // only do `i += 1` when nothing was removed.
    _ = list; // TODO: remove this discard once you use `list`.
    _ = value; // TODO: remove this discard once you use `value`.
    _ = keep_order; // TODO: remove this discard once you use `keep_order`.
    return 0;
}
...
pub fn insertSorted(alloc: std.mem.Allocator, list: *std.ArrayList(i64), value: i64) !void {
    // TODO: scan for the insertion index, then
...
test "clearRetainingCapacity empties the list but keeps the buffer" {
    ...
    // TODO: empty the list WITHOUT freeing its buffer (one method call).
    // deinit would free the memory; clearRetainingCapacity keeps it so
    // the next appends are allocation-free — great for per-line scratch
    // lists inside a parse loop.
