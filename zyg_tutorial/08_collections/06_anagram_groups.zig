...
pub fn sortedKey(alloc: std.mem.Allocator, word: []const u8) ![]u8 {
    // TODO: the dupe is done; sort its bytes before returning
    // (std.mem.sort over u8 with std.sort.asc — a string is just a
    // slice of bytes).
...
pub fn groupAnagrams(alloc: std.mem.Allocator, words: []const []const u8) !GroupMap {
    ...
    // TODO, for each word:
    //   1. const key = try sortedKey(alloc, word);
    //   2. const gop = try groups.getOrPut(key);
    //   3. if gop.found_existing: free YOUR key (the map keeps its own);
    //      else: initialize the uninitialized value: gop.value_ptr.* = .empty;
    //   4. try gop.value_ptr.append(alloc, word);
    // Then make sure deinitGroups (below) agrees with what you did —
    // read it before you start.
    _ = &groups; // TODO: remove this discard once you mutate `groups`.
    _ = words; // TODO: remove this discard once you use `words`.
    return groups;
}

/// PROVIDED: tear-down that honors the ownership contract above — free
/// every key we allocated, deinit every list, then the map itself. The
/// words inside the lists belong to the caller, so they are left alone.
/// Your groupAnagrams must produce a map this function can clean up.
