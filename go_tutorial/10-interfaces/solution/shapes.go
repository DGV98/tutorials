// Package interfaces contains the solutions for module 10: defining
...
// Compile-time checks: if either type ever stops satisfying Shape,
// these lines fail to build. The blank identifier means the values
...
func FilterShapes(shapes []Shape, keep func(Shape) bool) []Shape {
	// Appending to a nil slice is fine; no matches returns nil, which
	// the contract allows.
	var out []Shape
	for _, s := range shapes {
		if keep(s) {
