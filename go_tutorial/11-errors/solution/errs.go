...
func SafeDivide(a, b float64) (float64, error) {
	if b == 0 {
		// errors.New is enough here: the message is fixed, nothing
		// to format and no error underneath to wrap.
...
func ParseAge(s string) (int, error) {
	...
	if err != nil {
		// %w keeps the strconv error reachable for errors.Is/As;
		// %v here would flatten it into opaque text.
		return 0, fmt.Errorf("parse age %q: %w", s, err)
	}
	if n < 0 {
		return 0, fmt.Errorf("age %d must not be negative", n)
	}
	return n, nil
}
...
func Lookup(m map[string]int, key string) (int, error) {
	// The comma-ok form distinguishes "absent" from "stored zero";
	// it also reads safely from a nil map.
	v, ok := m[key]
	if !ok {
...
func ValidateUser(name string, age int) error {
	...
	if age < 0 || age > 150 {
		return &ValidationError{Field: "age", Reason: "must be between 0 and 150"}
	}
	return nil
}
...
func DontPanic(f func()) (err error) {
	defer func() {
		// recover returns the panic value, or nil if the function is
		// not panicking. Assigning to the named result err is how the
		// deferred closure changes what DontPanic returns.
		if v := recover(); v != nil {
