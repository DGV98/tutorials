...
func WordCount(text string) map[string]int {
	...
	for _, w := range strings.Fields(text) {
		counts[w]++ // missing keys read as 0, so no existence check needed
	}
	return counts
}
...
func Intersect(a, b []int) []int {
	...
	for _, x := range b {
		if _, ok := inA[x]; ok {
			...
			delete(inA, x) // so duplicates in b are not added twice
		}
	}
	slices.Sort(common)
	return common
...
func GroupAnagrams(words []string) [][]string {
	...
	// Map iteration order is random, so remember first-appearance order
	// of the keys separately.
	var order []string
	for _, w := range words {
		k := sortedLetters(w)
		if _, ok := groups[k]; !ok {
...
// sortedLetters returns w with its bytes sorted, the canonical form
// shared by all anagrams of w.
...
func Mode(xs []int) (int, bool) {
	...
	best, bestCount := xs[0], counts[xs[0]]
...
func Invert(m map[string]int) map[int]string {
	...
	for k, v := range m {
		// Iteration order is random: only overwrite when this key is
		// smaller than the one already recorded for v.
		if old, ok := inv[v]; ok && old < k {
			continue
		}
		inv[v] = k
	}
...
func TwoSum(xs []int, target int) (int, int, bool) {
	seen := make(map[int]int, len(xs)) // value -> index of its first occurrence
	for j, x := range xs {
		if i, ok := seen[target-x]; ok {
			return i, j, true
		}
		// Record only the first occurrence so ties resolve to the
		// smallest i. Checking before inserting also means x can never
		// pair with itself.
		if _, ok := seen[x]; !ok {
			seen[x] = j
		}
